...
bench_counts_path <- function(root) {
  path <- file.path(root, "inst", "extdata", "prek_sim_counts.rds")
  if (!file.exists(path)) {
    stop("Could not find source-tree prek_sim_counts.rds.", call. = FALSE)
  }
  path
}
...
bench_expected_case_specs <- function() {
  ...
  data.frame(
    case = c(
      ...
      "synthetic_c_medium",
      "synthetic_c_large",
      "prek_sim_scenario_b_counts_vjt",
      "prek_sim_aggregate_d0_frpm_vjt",
      "prek_sim_aggregate_d1_four_indicator_vjt"
    ),
    kind = c(rep("synthetic_c", 3L), rep("prek_sim", 3L)),
...
bench_validate_budget <- function(budget) {
  ...
  if (any(!budget$kind %in% c("synthetic_c", "prek_sim"))) {
...
bench_build_cases <- function(counts, budget) {
  prek_sim_rows <- unique(budget$S[budget$kind == "prek_sim"])
  if (length(prek_sim_rows) != 1L || nrow(counts) != prek_sim_rows) {
  ...
  cases$prek_sim_scenario_b_counts_vjt <- function() {
    sitemix::sm_estimate_from_counts(
      counts,
      family = "multivariate",
  ...
  cases$prek_sim_aggregate_d0_frpm_vjt <- function() {
  ...
  cases$prek_sim_aggregate_d1_four_indicator_vjt <- function() {
