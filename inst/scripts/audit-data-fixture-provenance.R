...
.data_fixture_public_paths <- c(
  "data/prek_sim.rda",
  "inst/extdata/prek_sim.csv",
  "inst/extdata/prek_sim_counts.rds",
  "inst/extdata/prek_sim_design.txt"
)

.data_fixture_review_paths <- file.path(
  "tests", "testthat", "_data", "regression",
  c(
    "prek_sim_summary.csv",
    "prek_sim_spotcheck_rows.csv",
    "prek_sim_spotcheck_vcov.csv",
...
.data_fixture_baseline_schema <- c(
  "metadata", "tolerances", "prek_sim_summary", "prek_sim_content",
  "scenario_a", "scenario_b", "scenario_c", "aggregate_d0", "aggregate_d1",
  "aggregate_d0_prek_sim_2024_frpm",
  "aggregate_d1_prek_sim_2024_four_indicator"
)

.data_fixture_expected_paths <- c(
  .data_fixture_public_paths,
  "inst/scripts/build-prek-sim.R",
  "tests/testthat/_data/regression/regression-baselines.rds",
...
.data_fixture_expected_metadata <- data.frame(
  producer = c(
    rep("inst/scripts/build-prek-sim.R", 4L),
    "maintainer_source",
    "inst/scripts/build-regression-baselines.R",
    rep(
  ...
  deterministic_seed = c(
    rep("declared_in_builder", 5L),
    rep("none", 7L)
  ),
  format = c(
    "rda_xz", "csv", "rds_v2_xdr", "plain_text",
    "R_script", "rds_v2_xdr", rep("csv", 5L), "R_script"
  ),
  regeneration_mode = c(
    rep("deterministic_rebuild_from_builder", 4L),
    "deterministic_rebuild_to_fresh_temp_only",
    "never_regenerate_in_automated_audit",
    rep("replay_review_csv_from_protected_baseline_only", 5L),
    "never_execute_in_automated_audit"
  ),
  restricted_input_required = rep("FALSE", 12L),
...
.data_fixture_helper_sha256 <-
  "d8995a4044f42164d593a998c23669e3bbbdd5b4ffd265d3704e385279d103db"
...
data_fixture_load_rda <- function(path) {
  ...
  if (!identical(names, "prek_sim")) {
    stop("Package-data RDA must contain exactly `prek_sim`.", call. = FALSE)
  }
  env$prek_sim
}
...
data_fixture_manifest <- function(root, source_arg) {
  ...
  if (nrow(manifest) != 12L) {
    issues <- c(issues, "manifest must contain exactly 12 rows")
  }
  ...
  if (sum(manifest$artifact_role == "restricted_canonical_input") != 0L) {
    issues <- c(issues, "manifest must not declare a restricted input")
  }
  ...
  allowed_missing <- rep(FALSE, nrow(manifest))
  ...
  manifest$observed_path_kind <- rep("source_root_relative", nrow(manifest))
...
data_fixture_public_contract <- function(root) {
  x <- data_fixture_load_rda(file.path(root, "data", "prek_sim.rda"))
  csv <- utils::read.csv(
    file.path(root, "inst", "extdata", "prek_sim.csv"),
    stringsAsFactors = FALSE,
    check.names = FALSE
  )
  counts <- readRDS(file.path(root, "inst", "extdata", "prek_sim_counts.rds"))
  provenance <- readLines(
    file.path(root, "inst", "extdata", "prek_sim_design.txt"),
    warn = FALSE
  )
  ...
  site_ids_generated <- all(grepl("^S[0-9]{3}$", x$site_id))
  student_ids_generated <- all(grepl("^ST[0-9]{5}$", x$student_id))
  student_ids_unique <- !anyDuplicated(x$student_id)
  design_note_present <- any(grepl(
    "prek_sim is a fully simulated panel",
    provenance,
    fixed = TRUE
  ))
  # the shipped design record must never acquire administrative-source
  # vocabulary; this guard fails loudly if a future edit reintroduces it
  provenance_clean <- !any(grepl(
    "restricted|confidential|administrative source|canonical source",
    provenance,
    ignore.case = TRUE
  ))
  residual_ok <- is.numeric(unlist(info$calibration_residual)) &&
    max(unlist(info$calibration_residual)) < 0.01
  rows <- list(
    data_fixture_contract_row(
      "package_data_dimensions",
      "7845x7",
      paste(dim(x), collapse = "x"),
      identical(dim(x), c(7845L, 7L))
    ),
    data_fixture_contract_row(
      "package_data_schema",
      package_schema,
    ...
    data_fixture_contract_row(
      ...
      "20260002",
      info$seed,
      identical(info$seed, 20260002L)
    ),
    data_fixture_contract_row(
      "site_size_seed",
      "909",
      info$size_seed,
      identical(info$size_seed, 909L)
    ),
    data_fixture_contract_row(
      "data_kind",
      "fully_simulated",
      info$data_kind,
      identical(info$data_kind, "fully_simulated")
    ),
    data_fixture_contract_row(
      "restricted_input_used",
      "FALSE",
      info$restricted_input_used,
      isFALSE(info$restricted_input_used)
    ),
    data_fixture_contract_row(
      "calibration_residual_within_tolerance",
      "< 0.01",
      residual_ok,
      residual_ok
    ),
    ...
    data_fixture_contract_row(
      "design_record_free_of_source_vocabulary",
      "TRUE",
      provenance_clean,
      provenance_clean
    )
...
# `prek_sim` is generated from design constants, so the audit has no external
# source to validate. The record below keeps the reporting shape stable for
# downstream consumers while stating plainly that no source is involved; a
# `--source` argument is accepted but deliberately ignored.
data_fixture_restricted_status <- function(manifest_result, source_arg, replay_requested) {
  data.frame(
    source_supplied = FALSE,
    source_available = FALSE,
    source_basename = "not_applicable",
    source_status = "NO_EXTERNAL_SOURCE_REQUIRED",
    identity_and_schema_exact = NA,
    replay_source_ready = TRUE,
...
data_fixture_public_replay <- function(
  ...
  replay <- c(
    file.path(data_dir, "prek_sim.rda"),
    file.path(extdata_dir, "prek_sim.csv"),
    file.path(extdata_dir, "prek_sim_counts.rds"),
    file.path(extdata_dir, "prek_sim_design.txt")
  )
  current_writer <- c(
  ...
  if (!replay_requested) {
    ...
    status <- system2(
      ...
      c(
        "--vanilla",
        shQuote(file.path(root, "inst", "scripts", "build-prek-sim.R")),
    ...
    # a clean rebuild emits no warnings at all
    expected_warning <- !any(grepl("^Warning message", log_lines))
...
data_fixture_baseline_contract <- function(root) {
  ...
  counts_path <- file.path(root, "inst", "extdata", "prek_sim_counts.rds")
  ...
  rows <- list(
    ...
    data_fixture_contract_row(
      ...
      identical(baseline$metadata$counts_file, "prek_sim_counts.rds")
    ),
    data_fixture_contract_row(
      "counts_md5",
      expected_counts_md5,
...
data_fixture_source_disposition <- function(source_build) {
  ...
  public_builder <- "inst/scripts/build-prek-sim.R"
  ...
  protected_baseline <-
    "tests/testthat/_data/regression/regression-baselines.rds"
  restricted_pattern <- "(?i)restricted[-_]?(source|input|panel)|confidential"
  restricted_present <- any(grepl(restricted_pattern, relative_contents, perl = TRUE))
...
data_fixture_source_manifest <- function(root) {
  relative <- c(
    ...
    "tests/testthat/test-data-prek-sim.R",
