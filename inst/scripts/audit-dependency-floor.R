...
dep_floor_smoke <- function() {
  result <- tryCatch({
    ...
    suppressWarnings(utils::data("prek_sim", package = "sitemix", envir = data_env))
    if (!exists("prek_sim", envir = data_env, inherits = FALSE)) {
      stop("installed prek_sim data is unavailable", call. = FALSE)
    }
    data <- get("prek_sim", envir = data_env, inherits = FALSE)
