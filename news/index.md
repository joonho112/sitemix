...
- **`alprek_subset` is replaced by `prek_sim`.** The example panel
  shipped with the package is now generated from design constants
  declared in `inst/scripts/build-prek-sim.R`. The builder reads no
  external file, so the panel describes no real children, sites, or
  program, and every shipped artifact can be regenerated from a plain R
  installation with no special data access.
- **Design targets are round numbers, not estimates.** Marginal rates,
  indicator correlations, between-site spread, and the three site-size
  strata are chosen to give the package a didactic example with the
  features its estimators are built for — site-year cells from a handful
  of children to more than a hundred, four overlapping means-tested
  indicators, one rare indicator that produces many zero cells, and
  enough between-site heterogeneity for a meaningful empirical-Bayes
  demonstration.
- **Provenance auditing simplified accordingly.** The data-fixture
  manifest drops its restricted-input row (13 rows to 12), and
  `audit-data-fixture-provenance.R` now verifies a full byte-exact
  rebuild of all four public artifacts instead of a conditional replay.
  Two disclosure guards were added: the shipped design record must not
  acquire administrative-source vocabulary, and no path in the built
  source package may look like a restricted input.
- **Breaking:** `alprek_subset`, `alprek_subset.csv`,
  `alprek_subset_counts.rds`, and `alprek_subset_provenance.txt` are
  removed. Use `prek_sim`, `prek_sim.csv`, `prek_sim_counts.rds`, and
  `prek_sim_design.txt`. Regression baselines and all printed vignette
  output were regenerated against the new panel.
- The A9 case study is no longer framed around a specific state program
  and is now
  [`vignette("a9-case-study-end-to-end")`](https://joonho112.github.io/sitemix/articles/a9-case-study-end-to-end.md).
...
- `prek_sim` — fully simulated 50-site pre-K panel used across examples,
  tests, and vignettes.
...
- `inst/extdata/prek_sim_counts.rds` for sufficient-count examples and
  regression tests.
...
- 17 vignettes split across an Applied Track (`a1` – `a9`) and a
  ...
  handoff, and an end-to-end case study.
- Bundled `prek_sim` `student_id` values are synthetic, sequential, and
  stable across years for the same selected child.
