test_that("prek_sim regression baselines remain stable", {
  ...
  expect_equal(current$prek_sim_summary, baseline$prek_sim_summary)
  expect_equal(current$prek_sim_content, baseline$prek_sim_content)
})
...
test_that("prek_sim 2024 aggregate regression pins match deterministic baselines", {
  baseline <- readRDS(testthat::test_path("_data", "regression", "regression-baselines.rds"))
  current <- regression_build_baselines()
  tol <- baseline$tolerances$scalar
  matrix_tol <- baseline$tolerances$matrix
  ...
  expect_equal(
    current$aggregate_d0_prek_sim_2024_frpm$attrs,
    baseline$aggregate_d0_prek_sim_2024_frpm$attrs
  )
  expect_equal(
    current$aggregate_d0_prek_sim_2024_frpm$summary,
    baseline$aggregate_d0_prek_sim_2024_frpm$summary,
    tolerance = tol
  )
  expect_equal(
    current$aggregate_d0_prek_sim_2024_frpm$sentinel_rows,
    baseline$aggregate_d0_prek_sim_2024_frpm$sentinel_rows,
    tolerance = matrix_tol
  )

  expect_equal(
    current$aggregate_d1_prek_sim_2024_four_indicator$attrs,
    baseline$aggregate_d1_prek_sim_2024_four_indicator$attrs
  )
  expect_equal(
    current$aggregate_d1_prek_sim_2024_four_indicator$by_indicator,
    baseline$aggregate_d1_prek_sim_2024_four_indicator$by_indicator,
    tolerance = tol
  )
  expect_equal(
    current$aggregate_d1_prek_sim_2024_four_indicator$sentinel_rows,
    baseline$aggregate_d1_prek_sim_2024_four_indicator$sentinel_rows,
    tolerance = matrix_tol
  )
  expect_equal(
    current$aggregate_d1_prek_sim_2024_four_indicator$vcov_pins,
    baseline$aggregate_d1_prek_sim_2024_four_indicator$vcov_pins,
    tolerance = matrix_tol
  )
  expect_lte(max(current$aggregate_d1_prek_sim_2024_four_indicator$vcov_pins$max_abs_offdiag), 1e-12)
})
