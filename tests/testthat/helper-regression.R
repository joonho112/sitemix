regression_counts_path <- function(root = NULL) {
  if (!is.null(root)) {
    candidate <- file.path(root, "inst", "extdata", "prek_sim_counts.rds")
  ...
  local_candidates <- c(
    file.path("inst", "extdata", "prek_sim_counts.rds"),
    file.path("..", "..", "inst", "extdata", "prek_sim_counts.rds")
  )
  ...
  installed <- system.file("extdata", "prek_sim_counts.rds", package = "sitemix")
  if (!nzchar(installed)) {
    stop("Could not find prek_sim_counts.rds.", call. = FALSE)
  }
  installed
}
...
regression_prek_sim_content <- function(counts, indicator_cols, pair_cols) {
...
regression_build_baselines <- function(root = NULL) {
  ...
  prek_sim_slice <- counts[seq_len(6L), , drop = FALSE]
  ...
  d0_input <- data.frame(
    site_id = prek_sim_slice$site_id,
    year = prek_sim_slice$year,
    indicator = "frpm",
    c_jt = prek_sim_slice$c_jt_frpm,
    n_jt = prek_sim_slice$n_jt,
    stringsAsFactors = FALSE
  )
  ...
  list(
    ...
    prek_sim_content = regression_prek_sim_content(counts, indicator_cols, pair_cols),
    ...
    aggregate_d0_prek_sim_2024_frpm = regression_d0_2024_frpm(counts_2024, sentinel_sites),
    aggregate_d1_prek_sim_2024_four_indicator = regression_d1_2024_four_indicator(counts_2024, indicators, sentinel_sites)
  )
}
...
regression_write_review_csvs <- function(baseline, out_dir) {
  ...
  summary_rows <- rbind(
    data.frame(section = "n_by_year", key = baseline$prek_sim_content$n_by_year$year, value = baseline$prek_sim_content$n_by_year$n_jt),
    data.frame(section = "n_jt_quantile_type1", key = baseline$prek_sim_content$n_jt_quantile_probs, value = baseline$prek_sim_content$n_jt_quantile_type1),
    data.frame(section = "d0_2024_frpm", key = names(baseline$aggregate_d0_prek_sim_2024_frpm$summary), value = unlist(baseline$aggregate_d0_prek_sim_2024_frpm$summary, use.names = FALSE)),
    data.frame(section = "d1_2024_four_indicator", key = paste0(baseline$aggregate_d1_prek_sim_2024_four_indicator$by_indicator$indicator, "_theta_raw_sum"), value = baseline$aggregate_d1_prek_sim_2024_four_indicator$by_indicator$theta_raw_sum),
    data.frame(section = "d1_2024_four_indicator", key = paste0(baseline$aggregate_d1_prek_sim_2024_four_indicator$by_indicator$indicator, "_theta_hat_sum"), value = baseline$aggregate_d1_prek_sim_2024_four_indicator$by_indicator$theta_hat_sum),
    data.frame(section = "d1_2024_four_indicator", key = paste0(baseline$aggregate_d1_prek_sim_2024_four_indicator$by_indicator$indicator, "_zero_cell"), value = baseline$aggregate_d1_prek_sim_2024_four_indicator$by_indicator$zero_cell)
  )
  utils::write.csv(summary_rows, file.path(out_dir, "prek_sim_summary.csv"), row.names = FALSE)

  spot_rows <- regression_bind_rows(
    list(
      data.frame(scenario = "d0_2024_frpm", baseline$aggregate_d0_prek_sim_2024_frpm$sentinel_rows, stringsAsFactors = FALSE),
      data.frame(scenario = "d1_2024_four_indicator", baseline$aggregate_d1_prek_sim_2024_four_indicator$sentinel_rows, stringsAsFactors = FALSE)
    )
  )
  utils::write.csv(spot_rows, file.path(out_dir, "prek_sim_spotcheck_rows.csv"), row.names = FALSE)
  utils::write.csv(
    baseline$aggregate_d1_prek_sim_2024_four_indicator$vcov_pins,
    file.path(out_dir, "prek_sim_spotcheck_vcov.csv"),
    row.names = FALSE
  )
}
